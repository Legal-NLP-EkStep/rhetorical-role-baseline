...
class BertData():
    ...
    def preprocess(self, src, tgt, sent_labels,sent_rhetorical_roles, use_bert_basic_tokenizer=False, is_test=False):
        ...
        sent_rr = [self.rhetorical_role_map.get(i) for i in sent_rhetorical_roles]
        sent_rr = sent_rr[:self.args.max_src_nsents]
        ...
        return src_subtoken_idxs, sent_labels,sent_rr, tgt_subtoken_idxs, segments_ids, cls_ids, src_txt, tgt_txt
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        f_name_chunk_id = '___'.join([d['source_filename'], str(d['src_chunk_id'])])
        sent_labels = greedy_selection(source[:args.max_src_nsents], tgt, len(tgt))
        sent_rhetorical_roles = d['src_rhetorical_roles']
        if (args.lower):
            source = [' '.join(s).lower().split() for s in source]
            tgt = [' '.join(s).lower().split() for s in tgt]
        b_data = bert.preprocess(source, tgt, sent_labels,sent_rhetorical_roles, use_bert_basic_tokenizer=args.use_bert_basic_tokenizer,
                                 is_test=is_test)
        ...
        b_data_dict = {"src": src_subtoken_idxs, "tgt": tgt_subtoken_idxs,
                       "src_sent_labels": sent_labels,"sentence_rhetorical_roles":sent_rr, "segs": segments_ids, 'clss': cls_ids,
