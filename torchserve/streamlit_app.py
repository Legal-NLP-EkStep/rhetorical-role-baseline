...
from torchserve.api_inference_helper import get_text_from_indiankanoon_url, get_predicted_rhetorical_roles
...
if indiankanoon_url!='':
    ...
    for i,sentence_dict in enumerate(output[0]['annotations'][0]['result']):
        ...
        if i < len(annotation_list) - 1 and annotation_list[i + 1]['value']['start'] > sentence_dict['value']['end']:
            sentences_with_rhetorical_roles.append((text[sentence_dict['value']['end']:annotation_list[i + 1]['value']['start']]))
